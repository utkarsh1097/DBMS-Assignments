...
void sequentialScan(int d)	//for the dimension
{
	...
	double second_closest = 0.0, farthest = 0.0;
	clock_t begin = clock();
	string filename = "queryset/queryset_"+to_string(d)+".txt";
	freopen(filename.c_str(), "r", stdin);
	string pathname = "seqscan_100NN/"+to_string(d);
	...
	average_time/=100;
	cout<<"Average time to find 100NN using sequential scan for dimension "<<d<<" is "<<average_time<<endl;
	fclose(stdout);
	freopen("ratio.txt", "a", stdout);
	cout<<double(second_closest/farthest)<<endl;
	fclose(stdout);
}
